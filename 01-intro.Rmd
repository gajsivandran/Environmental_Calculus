...
```{r lapse-rate-plot, echo=FALSE, fig.cap="Air temperature (°C) as a function of time (hours)", cache=TRUE}
plot(0, 0, type = "n", 
...
```{r apply-the-function-to-compute-temperature-405, fig.cap="Apply the function to compute temperature", cache=TRUE, echo=FALSE}
library(ggplot2)
library(dplyr)
...
$T(h) = 15 - 0.0065h$, where $h$ is elevation in meters.\
...
**b.** Does the increase look linear? Describe in words what you notice in the rate of increase. **c.** Interpolate the number of insects at 0.5 days. **d.** Extrapolate the number of insects on day 5. **e.** What is the difference between interpolation and extrapolation?

------------------------------------------------------------------------

**2.** The following graph shows temperature over a 24-hour period. Use the graph to answer the questions below.

```{r temp-day-curve, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Practice Problems", cache=TRUE}
...
**b.** When is it increasing most rapidly? When is decreasing most rapidly? **c.** Describe the concavity of this graph. **c.** What shape is this curve — linear, quadratic, exponential, or something else?
...
**7.** Below is a graph showing daily CO₂ concentration over a year. The trend shows a clear upward slope with regular seasonal fluctuations.
The black line represents the moving average (smooths out the variable) and the red line is the monthly averages.

![full dataset](bookdown-demo_files/figure-html/co2_data_mlo.png)
![recent trend data](bookdown-demo_files/figure-html/co2_trend_mlo.png)
**a.** Is this function linear, exponential, or neither?\
**b.** What causes the seasonal ups and downs?\
...
**9.**\
A table shows the dissolved oxygen (DO) level in a lake throughout the day:
...
**3.**\
A weather balloon starts at 1000 meters and rises at 150 meters per minute.\
...
-   Always include **units** in models, computations, and figure labels.
-   Use informative **figure captions** and cross-reference figures with chunk labels (e.g., Figure \@ref(fig:...)).
-   Prefer clean, named R chunks with `fig.cap` and `cache=TRUE` for reproducible, fast builds.

## Self-Check

1.  Explain the difference between a function’s **value** and its **rate of change** at a time $t$.
2.  If temperature drops by \~6.5 \u00B0C per 1000 m, what change do you expect from 500 m to 1800 m?
3.  Name two insights you get from a **contour plot** that a simple line plot does not provide.
