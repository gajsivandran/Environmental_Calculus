...
```{r, echo=FALSE}
# Placeholder for number line plot code
...
As we’ve seen, derivatives help us understand how a function **changes** — when it rises or falls, where it bends, and how quickly those changes happen.  
But some of the most important insights come from asking what happens **beyond** the range of normal behavior — as \( x \) becomes very large, very small, or approaches values where the function might “blow up.”  

These features, called **asymptotes**, describe how a system behaves at its **boundaries** — either far into the future, under extreme conditions, or near critical thresholds where the function is undefined.
...
They can also signal **thresholds** where a variable diverges or becomes undefined, such as at a vertical boundary.

Examples:

- **Logistic population models** approach a **horizontal asymptote** at the **carrying capacity** — growth slows but never exceeds it.  
- **Pollution concentration** may approach zero over time after cleanup, modeling a **decay asymptote**.  
- **Runoff models** or **rational functions** can have **vertical asymptotes** representing physical or mathematical boundaries, like when infiltration becomes undefined.  
- **Temperature–altitude relationships** may level off at high altitudes, showing an **approaching limit** rather than a constant slope.

In short: asymptotes describe the **long-term behavior** or **boundary conditions** of change.
...
|------|--------------|---------------|
| **Vertical Asymptote** | Function value grows toward \( \pm\infty \) near some \( x = a \) | Examine denominator → does \( f(x) \to \infty \) or \( -\infty \) as \( x \to a \)? |
...
| **Oblique (Slant) Asymptote** | Function approaches a **line** (not horizontal) as \( x \to \pm\infty \) | Divide numerator by denominator → remainder becomes small |

Each type tells us how a curve behaves at the “edges” — near undefined points or far from the origin.
...
Look for where the function is **undefined** (typically where the denominator equals zero).  
Then check the **limit** as \( x \) approaches that point from both sides.
...
These describe the **end behavior** of the function — what happens as \( x \to \infty \) or \( x \to -\infty \).
...
In environmental terms, this could model a pollutant concentration that rises quickly, then stabilizes at a long-term plateau.

---

#### **3. Oblique (Slant) Asymptotes**

If the degree of the numerator is **one higher** than the denominator, the function approaches a **slanted line** as \( x \to \infty \).

To find it, use **long division** or **polynomial division**:

\[
f(x) = \frac{N(x)}{D(x)} = Q(x) + \frac{R(x)}{D(x)}
\]
As \( x \to \infty \), \( \frac{R(x)}{D(x)} \to 0 \),  
so \( y = Q(x) \) is the **oblique asymptote**.

> **Example:**  
> \( f(x) = \dfrac{x^2 + x + 1}{x + 1} = x + 0 + \dfrac{1}{x + 1} \)  
> As \( x \to \infty \), \( \dfrac{1}{x + 1} \to 0 \) → \( y = x \) is the **slant asymptote**.

---

### Visualizing the Three Asymptote Types

```{r asymptote-types, echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=8}
library(ggplot2)
library(patchwork)

# 1. Vertical asymptote
f1 <- function(x) 1 / (x - 2)
x1 <- seq(0.5, 3.5, length.out = 400)
df1 <- data.frame(x = x1, y = f1(x1))
p1 <- ggplot(df1, aes(x, y)) +
  geom_line(color = "steelblue", linewidth = 1.2) +
  geom_vline(xintercept = 2, linetype = "dashed", color = "gray50") +
  labs(title = "A. Vertical Asymptote at x = 2", subtitle = expression(f(x) == 1 / (x - 2))) +
  theme_minimal(base_size = 12)

# 2. Horizontal asymptote
f2 <- function(x) (3*x^2 + 5) / (x^2 + 1)
x2 <- seq(-5, 5, length.out = 400)
df2 <- data.frame(x = x2, y = f2(x2))
p2 <- ggplot(df2, aes(x, y)) +
  geom_line(color = "darkgreen", linewidth = 1.2) +
  geom_hline(yintercept = 3, linetype = "dashed", color = "gray50") +
  labs(title = "B. Horizontal Asymptote y = 3", subtitle = expression(f(x) == (3*x^2 + 5) / (x^2 + 1))) +
  theme_minimal(base_size = 12)

# 3. Slant asymptote
f3 <- function(x) (x^2 + x + 1) / (x + 1)
x3 <- seq(-6, 6, length.out = 400)
df3 <- data.frame(x = x3, y = f3(x3))
p3 <- ggplot(df3, aes(x, y)) +
  geom_line(color = "purple", linewidth = 1.2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray50") +
  labs(title = "C. Oblique Asymptote y = x", subtitle = expression(f(x) == (x^2 + x + 1) / (x + 1))) +
  theme_minimal(base_size = 12)
...
Each panel highlights a different kind of boundary behavior:

- **A.** The curve shoots to infinity near \( x = 2 \) — a **vertical asymptote**, representing a boundary or threshold.  
- **B.** The curve flattens toward a stable value (\( y = 3 \)) — a **horizontal asymptote**, showing a long-term limit.  
- **C.** The curve approaches a tilted line (\( y = x \)) — an **oblique asymptote**, showing a directional trend that continues indefinitely.

---

### Environmental Interpretation

Asymptotes reveal *how a process behaves at its edges* — whether it stabilizes, diverges, or trends indefinitely.

| Type | Example | Environmental Meaning |
|------|----------|------------------------|
| **Vertical** | \( f(x) = \frac{1}{x-2} \): undefined at \( x=2 \) | A **threshold** or boundary where a system variable becomes infinite or undefined (e.g., pollutant concentration diverging near saturation). |
| **Horizontal** | \( f(x) = \frac{3x^2+5}{x^2+1} \): \( y \to 3 \) | A **steady-state limit** — like population or CO₂ levels approaching equilibrium. |
| **Oblique** | \( f(x) = \frac{x^2+x+1}{x+1} \): \( y \to x \) | A **trend-driven process**, such as temperature anomaly rising along a consistent long-term slope. |

---

### How This Complements Derivatives

- The **first derivative** shows **where change stops or reverses** (extrema).  
- The **second derivative** shows **how change bends** (concavity).  
- **Asymptotes** show **what happens at the limits** — far beyond the local details.

Together, they create a full picture of system behavior:
- Near critical points → we study slope and curvature.  
- At long-term or extreme values → we study asymptotes.  

This combination helps us model systems that are both **bounded** (limited by resources) and **unbounded** (driven by ongoing forces).

---

### Quick Practice — Reading Asymptotes from a Function

For each function below, identify any asymptotes and classify their type.
...
| Function | Asymptotes | Type |
|-----------|-------------|------|
| \( \dfrac{2x^2 - 1}{x^2 + 1} \) | \( y = 2 \) | Horizontal (approaches a limit at infinity) |
| \( \dfrac{1}{x - 4} \) | \( x = 4 \) | Vertical (undefined at boundary) |
| \( \dfrac{x^2 + 2x + 1}{x + 1} = x + 1 + \dfrac{0}{x+1} \) | \( y = x + 1 \) | Oblique (approaches a slant line) |
| \( 5 - e^{-x} \) | \( y = 5 \) as \( x \to \infty \) | Horizontal (approaches equilibrium) |
...
1. How do asymptotes help you anticipate long-term or extreme behavior in environmental models?  
2. Why might a **horizontal asymptote** represent stability, while a **vertical asymptote** represents a system boundary or breakdown?  
3. How does combining **derivatives** and **asymptotes** create a richer understanding of environmental change?

> **Big Idea:**  
> Derivatives describe how systems **change** in the short term.  
> Asymptotes describe where those systems are **headed** in the long term — the limits, thresholds, and steady states that define environmental balance.
...
|-----------|------------------|
| \( f(x) = x^3 - 3x \) | \( f''(x) = 6x \) |
| \( f(x) = \frac{x^3}{3} - x \) | \( f''(x) = 2x \) |
| \( f(x) = 0.2x^5 - x^3 \) | \( f''(x) = 4x^3 - 6x \) |
