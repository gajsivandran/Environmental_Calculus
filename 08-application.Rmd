...
![Figure: Examples of local and global extrema across different types of functions.](images/extrema_examples.png){fig.alt="Four-panel figure showing different behaviors of extrema. Panel A shows a cubic function with one local maximum and one local minimum marked on the curve. Panel B shows a downward-opening parabola on a finite interval, with a global maximum at the peak and endpoint minima at both ends. Panel C shows a cubic function with a flat point at x = 0 where the slope is zero but there is no maximum or minimum. Panel D shows a sine wave over one full period from 0 to 2π, with a repeating local maximum near π/2 and a local minimum near 3π/2."}
...
### Why “Critical Points” Aren’t Always Extremes
...
#### When Flat Doesn’t Mean Peak or Valley
...
![Figure: Relationship between a function and its derivative for a cubic function.](images/cubic_derivative_relationship.png){fig.alt="Two side-by-side graphs comparing a function and its derivative. The left graph shows f(x) = x cubed, an S-shaped curve that flattens near x = 0 and increases for all x. The right graph shows its derivative f prime of x equals 3x squared, a parabola opening upward with its minimum at x = 0. The derivative is zero at x = 0, corresponding to the flat point on the original function where the slope is zero but there is no maximum or minimum."}
...
1. **Take the derivative** of the function.  
   → This gives you a formula for the **slope** (how fast the function is changing).

2. **Find critical points** by solving \( f'(x) = 0 \) (or where \( f'(x) \) is undefined).  
   → These are *possible* peaks or valleys.

3. **Use a sign chart or second derivative** to classify each critical point.  
   → This tells you whether each point is a **maximum**, **minimum**, or **neither**.
...
At a peak or valley, the tangent line to the curve is **flat** — no uphill or downhill slope — meaning:
\[
f'(x) = 0
\]
...
You can think of the sign chart as a slope “map”:

- \( + \) means the function rises (↗️)
- \( - \) means it falls (↘️)
...
![Figure: Population modeled by a cubic function over time.](images/population_cubic.png){fig.alt="Graph of population P(t) equals negative t cubed plus 6t squared minus 9t plus 20 plotted over time from t equals 0 to 5. The curve starts near 20 at t equals 0, decreases to a local minimum around t equals 1, then increases to a local maximum near t equals 3 close to 20, and finally declines steeply toward zero by t equals 5. The horizontal axis represents time and the vertical axis represents population."}
...
![Figure: Number line showing critical points and intervals.](images/critical_points_numberline.png){fig.alt="A horizontal number line with arrows at both ends indicating it continues indefinitely. Two marked points at x equals 1 and x equals 3 are labeled as critical points. The intervals between and around these points are labeled A to the left of 1, B between 1 and 3, and C to the right of 3, indicating three regions for analysis around the critical points."}
...
>  
> The derivative’s sign tells the whole story of how the population grows and declines over time.
...
![Figure: Cubic model showing local minimum and maximum.](images/cubic_extrema_labeled.png){fig.alt="Graph of a cubic population model over time from 0 to 5. The curve starts near 20, decreases to a labeled local minimum at time t equals 1 with population 16, then rises to a labeled local maximum at time t equals 3 with population 20, and finally decreases steeply toward zero by time t equals 5. Dashed lines mark the x and y coordinates of the extrema. The horizontal axis represents time and the vertical axis represents population."}
...
# ----------------------------------------------------------
# D. Multiple inflection points (0.2x⁵ − x³)
...
![Figure: Examples of concavity and inflection behavior for different functions.](images/concavity_examples.png){fig.alt="Four-panel figure illustrating concavity. Panel A shows f(x) equals x cubed with a change in concavity at x equals 0 marked as an inflection point, with the graph concave down for negative x and concave up for positive x. Panel B shows f(x) equals x squared, which is concave up for all x. Panel C shows f(x) equals negative x squared, which is concave down for all x. Panel D shows a higher-degree polynomial with multiple inflection points where the graph alternates between concave up and concave down across several intervals."}
...
- **Concave Down** → shaped like a **hill** (opens downward)  
...
2. **Set \( f''(x) = 0 \)** (or find where it’s undefined) — these are potential **inflection points**.  
...
p4 <- plot_inflection(f5, f2_5, c(-4,4), 0,
                      "D. Steep Inflection — Rapid Concavity Change",
...
![Figure: Different types of inflection points and concavity changes.](images/inflection_types.png){fig.alt="Four-panel figure showing different types of inflection points. Panel A shows f(x) equals x cubed with a standard inflection at x equals 0 where concavity changes and the slope is nonzero. Panel B shows f(x) equals x to the fifth with a flat inflection at x equals 0 where the slope is zero and the graph flattens before changing concavity. Panel C shows a shifted cubic with an inflection point near x equals 2 where the slope is nonzero. Panel D shows f(x) equals hyperbolic tangent of x with a steep inflection at x equals 0 where concavity changes rapidly. In each panel, the inflection point is marked and the graph changes color to indicate concavity on either side."}
...
### Using L’Hôpital’s Rule to Evaluate Asymptotic Limits
...
1. **Check the form.** Confirm it’s \( 0/0 \) or \( ∞/∞ \).  
2. **Differentiate numerator and denominator separately.**  
3. **Re-evaluate the limit.** If it’s still indeterminate, apply the rule again.
...
Both → 0. Differentiate:

\[
\lim_{x \to 0} \frac{\cos x}{1} = 1.
\]

The ratio stabilizes to 1 — showing smooth near-zero behavior, not a blow-up.

---

#### Example 3 — Environmental Ratio {-}

\[
\lim_{t \to \infty} \frac{e^{-0.1t}}{e^{-0.3t}}.
\]

Both → 0. Apply the rule:
...
No horizontal asymptote — uptake dominates decay as time grows.
...
### Quick Practice — Asymptotes & Limits
...
1. How does L’Hôpital’s Rule connect **derivatives** to **asymptotic limits**?  
...
3. How can evaluating these limits guide environmental interpretations — like steady-state pollution, population plateaus, or saturation thresholds?
...
# 2. Quadratic with defined interval — endpoints matter
...
# 3. Sinusoidal — repeated local extrema
f3 <- function(x) sin(x)
...
![Figure: Examples of functions with different types of extrema and domains.](images/extrema_variety.png){fig.alt="Four-panel figure showing different functions and their extrema. Top left shows a cubic function f(x) equals x cubed minus 3x squared plus 2 with one local maximum near x equals 0 and one local minimum near x equals 2. Top right shows a downward-opening parabola f(x) equals negative x squared plus 4x on the interval from 0 to 5, with a maximum near x equals 2 and endpoints at x equals 0 and x equals 5. Bottom left shows f(x) equals sine of x over one full period from 0 to 2π with a maximum near π over 2 and a minimum near 3π over 2. Bottom right shows a quartic function f(x) equals x to the fourth minus 4x squared plus 2 with two local minima on either side of the origin and a local maximum at x equals 0."}
...
2. What does a “flat slope” or “bend change” mean in a real environmental context?  
3. Why do scientists care about *when* a rate of change stops increasing — even before it becomes negative?
...
- A **curvature switch** (sign change in \( f'' \)) indicates a transition in *how* the system is changing (acceleration ↔ deceleration).  
...
2. A **flat slope** (\( f'(x)=0 \)) marks a turning point; a **bend change** (sign change in \( f'' \)) marks a shift in acceleration — both are diagnostically different.  
3. Scientists watch for where the rate of change **stops increasing** ( \( f''\) changes sign) because it often precedes critical thresholds — a chance for **early intervention**.
