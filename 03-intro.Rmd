...
kable(
  temp_df,
  col.names = c("Time (hours)", "Elevation (m)", "Temperature (degC)"),
...
```{r apply-the-function-to-compute-temperature-40, fig.cap="Temperature as a function of time of day and elevation.", fig.alt="Filled contour plot showing temperature across time of day and elevation. The horizontal axis shows time from 0 to 24 hours, and the vertical axis shows elevation from 0 to 5000 meters. Colored contour bands represent temperature in degrees Celsius. Temperature varies smoothly across the surface, showing how it changes jointly with time and elevation.", cache=FALSE, echo=FALSE, message=FALSE}
...
```{r temperature-time-elevation-lines, fig.cap="Temperature over time at multiple elevations.", fig.alt="Interactive line plot showing temperature as a function of time of day for several elevations. The horizontal axis shows time from 0 to 24 hours, and the vertical axis shows temperature in degrees Celsius. Multiple colored lines represent different elevations ranging from 0 meters to 4392 meters. Each curve shows a repeating daily temperature cycle, and higher elevations have consistently lower temperatures than lower elevations.", echo=FALSE, message=FALSE}
...
```{r graph-change-example, fig.cap="Quadratic function showing a maximum.", fig.alt="Graph of a downward-opening parabola representing the function f(x) equals negative x squared plus 4x plus 5 over the interval from 0 to 6. The curve rises to a highest point near x equals 2 and then decreases. A dashed horizontal line marks y equals 0. The graph illustrates that the function increases up to the maximum and then decreases afterward.", echo=FALSE}
...
```{r temp-day-curve, fig.cap="Air temperature over a 24-hour period.", fig.alt="Line graph showing temperature as a function of time over a 24-hour day. The horizontal axis shows time from 0 to 24 hours, and the vertical axis shows temperature in degrees Celsius. The curve follows a smooth repeating pattern, with a minimum temperature around early morning and a maximum near midday at 12 hours, marked by a vertical dashed line. The graph illustrates a typical daily temperature cycle.", echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE}
