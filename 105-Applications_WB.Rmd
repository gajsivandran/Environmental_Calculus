...
These are vertical asymptote candidates.

Sign chart:

- Increasing on \((-∞,-1)\) and \((1,3)\)
- Decreasing on \((-1,0)\), \((0,1)\), and \((3,∞)\)
...
The numerator has **one real root**, found numerically:

\[
x \approx -2.599
\]

This is the **only inflection point**.
...
|----------|------------------|-----------|
| \((-\infty, -2.599)\) | – | **Concave down** |
| \((-2.599, 0)\) | + | **Concave up** |
| \((0, 3)\) | – | **Concave down** |
| \((3, \infty)\) | + | **Concave up** |

Thus concavity alternates:
\[
\text{down → up → down → up}
\]
...
So \(x=3\) is also a **vertical asymptote**.
...
\[
f''(x)=\frac{6(x^3 + x^2 - 3x + 3)}{x^3 (x-3)^3}
\]
...
Concavity pattern:
- **Concave down** on \((-\infty,-2.599)\)
- **Concave up** on \((-2.599,0)\)
- **Concave down** on \((0,3)\)
- **Concave up** on \((3,\infty)\)

This alternating pattern reflects the influence of both asymptotes.
...
- One inflection point near \(x=-2.599\)  
- Concave down → up → down → up pattern  
- Curve breaks naturally into three disconnected pieces  
...
infl <- -2.599   # approx numeric root

# Build 3 separate data frames to avoid connecting across asymptotes
df1 <- data.frame(x = seq(-6, -0.01, length.out = 800))
df2 <- data.frame(x = seq(0.01, 2.99, length.out = 800))
df3 <- data.frame(x = seq(3.01, 6, length.out = 800))
...
ggplot() +
  ...
  # Curve pieces plotted separately (NO connecting)
  ...
  # Critical points
  geom_point(aes(x = crit1, y = f(crit1)), color = "darkgreen", size = 3) +
  geom_point(aes(x = crit2, y = f(crit2)), color = "darkgreen", size = 3) +
  annotate("text", x = crit1, y = f(crit1), label = "local max", vjust = -1) +
  annotate("text", x = crit2, y = f(crit2), label = "local min", vjust = -1) +
  
  # Inflection point
  geom_point(aes(x = infl, y = f(infl)), color="purple", size=3) +
  annotate("text", x = infl, y = f(infl), label = "inflection", vjust = 1.5) +
  
  labs(
    title = "Challenge 2:  f(x) = (x^2 + 1) / (x(x - 3))",
    subtitle = "Concavity shading (blue = up, gold = down) with asymptotes at x=0 and x=3",
    x = "x",
    y = "f(x)"
  ) +
  
  # Set your own y-limits here
