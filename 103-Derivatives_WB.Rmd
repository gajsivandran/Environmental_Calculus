...
::: promptbox
**Activity:**  
![QR code for shared google doc](images/change_brainstorm.png){fig.alt="QR code linking to a shared Google Doc for the change brainstorm activity." style="float:right; width:250px; margin:10px;"}
...
$$f(x) = -x^2 + 4x + 5$$
```{r avg-rate-quad, fig.cap="Quadratic function used to explore average rate of change.", fig.alt="Graph of a downward-opening quadratic function f(x) equals negative x squared plus 4x plus 5 over the interval from 0 to 6. The curve rises to a maximum and then decreases, forming a smooth parabola. The figure provides a visual context for analyzing average rate of change over different intervals.", echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
...
**Notes:** <div class="writein tall"></div>
:::
...
```{r zoom-demo, fig.cap="Zooming in on a point to study local behavior of a curve.", fig.alt="Graph of a smooth upward-opening curve representing sea level over time from about 1995 to 2025. A red point is marked at the year 2010 on the curve. The figure illustrates how focusing on a small region around a point helps analyze local behavior, such as estimating the slope or rate of change near that point.", echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
...
# Draw tangents
cols <- c("darkgreen", "black", "firebrick")
for (i in 1:3) {
  draw_tangent(pts$x[i], pts$y[i], pts$m[i], cols[i])
  points(pts$x[i], pts$y[i], pch = 19, col = cols[i])
  text(pts$x[i], pts$y[i], paste(" ", pts$lab[i]), pos = 4, col = cols[i])
}
```
...
**Notes:** <div class="writein tall"></div>
**Challenge**: Can you sketch the derivative - don't worry about getting the right magnitudes, just the right shape. [Hint: start with some points you know for sure, then ask yourself as you move left to right what is happening to the slope]

```{r tangent-slope-and-blank-derivative, fig.cap="Using tangent slopes to sketch the derivative.", fig.alt="Two stacked panels show a function and a blank coordinate grid for its derivative. The top panel shows a smooth curve over the interval from 0 to 10 with three marked points. Dashed tangent lines at those points show one positive slope, one zero slope, and one negative slope, illustrating how the sign of the derivative depends on local behavior of the function. The bottom panel is a blank graph labeled f prime of x with horizontal and vertical reference lines, intended for sketching the derivative based on the slope information from the top panel.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r daylight-secant-to-tangent, fig.cap="Secant lines approaching a tangent line for a daylight model.", fig.alt="Graph of daylight hours over time modeled by a smooth periodic curve across two years. A point at a chosen day a is marked on the curve. Several secant lines connect this point to nearby points at increasing proximity, showing how the slope of the secant lines approaches the slope of the tangent line at that point. The figure illustrates how average rates of change approach the instantaneous rate of change as the interval shrinks.", echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.height=5, fig.width=8, out.width='90%'}

# Daylight model for Seattle (simple sinusoid with correct phase)
omega <- 2*pi/365
D <- function(t) 12.25 + 3.75*cos(omega*(t - 172))

# Choose the focal day 'a'
a <- 250
...
# Draw secant lines
cols <- c("#999999", "#777777", "#555555", "#333333", "#000000")
...
for (i in seq_along(h_vals)) {
  ...
  xline <- seq(x0 - 50, x0 + 50, length.out = 200)
  yline <- y0 + slope*(xline - x0)
  
  lines(xline, yline, col = cols[i], lwd = 1.5, lty = 2)
}
```
...
Sometimes \(f(a)\) is undefined (like dividing by zero). In fact - when we look at the limit definition of a derivative, this is the problem we encounter.
...
```{r smoothness-4panels, fig.cap="Examples showing different kinds of smoothness and non-smooth behavior.", fig.alt="Four-panel figure comparing functions with different local behaviors at x equals 0. Panel A shows the parabola x squared, which is smooth and differentiable at the origin. Panel B shows the absolute value function, which has a sharp corner at the origin and is not differentiable there. Panel C shows the cube root function, which passes through the origin with a vertical tangent and is not differentiable there. Panel D shows the function x times sine of 1 over x with f of 0 defined as 0; the graph is continuous at the origin but oscillates rapidly nearby, illustrating complicated local behavior. In each panel, the point at x equals 0 is highlighted to compare how continuity does not always imply smoothness.", echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=7}
...
```{r differentiability-new-4panels, fig.cap="Examples of differentiable and non-differentiable behavior at a point.", fig.alt="Four-panel figure comparing functions at x equals 0. Panel 1 shows the function e to the negative x squared, which is smooth and differentiable at x equals 0. Panel 2 shows the function square root of the absolute value of x, which has a sharp vertical cusp at x equals 0 and is not differentiable there. Panel 3 shows the function x plus the absolute value of x, which has a corner at x equals 0 and is not differentiable there. Panel 4 shows the function sine of x squared, which is smooth and differentiable at x equals 0. In each panel, the point at x equals 0 is highlighted to compare how some continuous functions are differentiable while others are not.", echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=7}
...
```{r piecewise-linear-discontinuous, fig.cap="Piecewise linear function with discontinuities at junctions.", fig.alt="Graph of a piecewise linear function made of three line segments over the interval from negative 5 to 5. The left segment decreases and ends with a closed point at x equals negative 2. The middle segment increases gently and begins with an open circle at x equals negative 2, then ends with a closed point at x equals 1. The right segment decreases and begins with an open circle at x equals 1. The open and closed circles show jump discontinuities at the transition points.", echo=FALSE, message=FALSE, warning=FALSE}
...
```
```{r wavy-function-continuous, fig.cap="Smooth continuous function with changing slope and curvature.", fig.alt="Graph of a smooth continuous function over the interval from negative 5 to 5. The curve rises and falls multiple times, showing regions of increasing and decreasing behavior and changing concavity. There are no breaks, holes, or jumps in the graph, illustrating a function that is continuous everywhere on the interval.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r sine-function-basic, fig.cap="Sine function over multiple periods showing periodic behavior.", fig.alt="Graph of the sine function from x equals 0 to x equals 4 pi. The curve oscillates smoothly above and below the x-axis, forming repeating peaks and troughs. The function crosses the x-axis at regular intervals, illustrating its periodic nature.", echo=FALSE, message=FALSE, warning=FALSE}
...
points(-1, -0.5*(-1+4)^2 + 1, pch=16, col="darkred")
points(-1, sin(1.5*(-1)) + 0.5*(-1), pch=1, col="darkred")
points(1.5, sin(1.5*1.5) + 0.5*1.5, pch=16, col="darkred")
points(1.5, 0.1*(1.5-1.5)^3 - 0.4*(1.5-1.5)^2 + 2, pch=1, col="darkred")
...
1. What is the water temperature right at the dam ($x = 0$)?
<div class="writein"></div>

2. What happens to the temperature as you go far downstream ($x \to \infty$)?
<div class="writein"></div>

3. Use the limit definition (or your intuition) to describe: is the temperature changing *faster* near the dam or far downstream? How do you know?
<div class="writein"></div>

4. Sketch what the derivative $T'(x)$ might look like. Is it positive or negative? Increasing or decreasing?
<div class="writein tall"></div>

5. At what distance downstream would you expect the temperature to be changing most rapidly? What ecological implication does this have for fish and aquatic organisms?
<div class="writein"></div>
:::

---

### Wrapping Up: What Is a Derivative?

::: promptbox
**Synthesis**

In your own words, complete these sentences:

1. A derivative tells us...
<div class="writein"></div>

2. The difference between an average rate and an instantaneous rate is...
<div class="writein"></div>

3. One environmental system where derivatives matter is... because...
<div class="writein"></div>
:::
