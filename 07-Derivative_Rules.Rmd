...
legend("bottomright", legend = c("f(x)", "f'(x)"),
...
```{r constant-rule-plot-fixed, fig.cap="Constant function and its derivative.", fig.alt="Graph showing a constant function f(x) equals 4 and its derivative f prime of x equals 0 over the interval from negative 5 to 5. The function is shown as a horizontal line at y equals 4. The derivative is shown as a dashed horizontal line at y equals 0. The figure illustrates that a constant function has zero rate of change everywhere.", echo=FALSE, fig.width=6, fig.height=4}
...
```{r constant-multiple-rule-plot-2, fig.cap="Constant multiple rule illustrated with a cubic function.", fig.alt="Graph showing the function f(x) equals 7x cubed and its derivative f prime of x equals 21x squared over the interval from negative 3 to 3. The function is shown as a blue curve passing through the origin with increasing steepness. The derivative is shown as a red upward-opening parabola that is always nonnegative and equals zero at x equals 0. The figure illustrates that multiplying a function by a constant scales its derivative by the same constant, as seen in the relationship between 7x cubed and 21x squared.", echo=FALSE, fig.width=6, fig.height=4}
...
```{r exp-clean-2point2s, fig.cap="Exponential function with tangent lines at two points.", fig.alt="Graph of the exponential function y equals e to the x over the interval from negative 2 to 2. Two tangent line segments are drawn, one at x equals 0 and one at x equals 1. At each point, the tangent line touches the curve and has the same slope as the function. The labels show that the value of the function equals the value of its derivative at each point, illustrating that the derivative of e to the x is itself. The tangent lines closely match the curve near their points of contact but diverge as x moves away.", echo=FALSE, fig.width=7, fig.height=5}
...
At $t = 10$:
$$N'(10) = 50e^{1} \approx 50(2.718) \approx 136 \text{ bacteria per hour}$$
...
- a) is just a constant multiple of a power — use the constant multiple rule
- b) is a product of two *different types* of functions ($x^2$ and $e^x$)
- c) is a product of two polynomial expressions
- d) is just a constant multiple of $\ln(x)$ — use the constant multiple rule
...
At $t = 0$, $D'(0) = 0$ \u2014 the dissolved oxygen isn't changing. For $t > 0$, $D'(t) > 0$ \u2014 DO is increasing, but at a decreasing rate.
...
```{r contour-energy-map, fig.cap="Contour map of E(x, y) = x^2 + y^2.", fig.alt="Contour plot of the function E(x, y) equals x squared plus y squared over a square region from negative 5 to 5 in both directions. The contours are concentric circles centered at the origin, with increasing values moving outward. Labeled contour levels include 1, 4, 9, 16, and 25. A point at coordinates (3, 4) is marked and labeled E equals 25, showing that all points on the same circle have the same function value. The figure illustrates how contour lines represent constant values of a function in two variables.", echo=FALSE}
