...
plot(t_vals, temp_vals, type = "l", lwd = 2, col = "blue",
     ylab = "Temperature (degC)", xlab = "Time (hours)",
     main = "Lake Temperature T(t)")

# Plot derivative
plot(t_vals, deriv_vals, type = "l", lwd = 2, col = "red",
     ylab = "Rate of Change (degC/hour)", xlab = "Time (hours)",
     main = "Derivative T'(t)")
abline(h = 0, col = "gray", lty = 2)
```

![Figure: Lake temperature and its derivative over time.](images/sea_level.png){fig.alt="Two stacked graphs show lake temperature and its rate of change over time in hours. The top graph shows temperature oscillating smoothly between about 16°C and 20°C in a repeating wave pattern over roughly 48 hours. The bottom graph shows the derivative, representing rate of change, also oscillating as a wave. When the temperature graph is increasing, the derivative is positive; when the temperature is decreasing, the derivative is negative; and when the temperature reaches peaks or troughs, the derivative crosses zero. The graphs illustrate the relationship between a function and its derivative."}
...
![Figure: Function and its derivative.](images/sea_level.png){fig.alt="A graph shows two curves plotted together: a black curve representing f(x) and a red curve representing its derivative f'(x). The black curve has a local maximum on the left and a local minimum on the right. The red curve crosses the horizontal axis at the x-values corresponding to these turning points. Where the black curve is increasing, the red curve is above zero; where the black curve is decreasing, the red curve is below zero. The graph illustrates the relationship between a function and its derivative."}
...
![Figure: Constant function and its derivative.](images/sea_level.png){fig.alt="A graph shows two horizontal lines: a solid line at y = 4 representing the constant function f(x) = 4, and a dashed line at y = 0 representing its derivative f'(x) = 0. Both lines extend across all x-values. The graph illustrates that the derivative of a constant function is zero everywhere."}
...
![Figure: Constant multiple rule example with function and derivative.](images/sea_level.png){fig.alt="A graph shows two curves: a blue curve representing the function f(x) = 7x cubed and a red dashed curve representing its derivative f'(x) = 21x squared. The blue curve has an S-shape, decreasing for negative x and increasing for positive x, with a flat point at x = 0. The red dashed curve is a parabola opening upward, with its minimum at x = 0. The graph illustrates how the derivative of 7x cubed is 21x squared, demonstrating the constant multiple rule."}
...
![Figure: Exponential function with tangent lines at x = 0 and x = 1.](images/sea_level.png){fig.alt="A graph shows the exponential function y = e to the x as a smooth increasing blue curve. Two points are highlighted: (0, 1) and (1, about 2.72). At each point, a dashed line segment represents the tangent line, showing the slope of the curve at that point. The tangent at x = 0 is less steep, while the tangent at x = 1 is steeper, illustrating how the rate of change increases as x increases. Labels indicate the function values at each point."}
...
### When Do We Need Implicit Differentiation?
...
### Conceptual Analogy: Contour Lines on a Map
...
```{r, echo=FALSE}
# This R code generates the contour plot shown below
# (Code is suppressed for students in the final version)

x <- seq(-5, 5, length.out = 100)
y <- seq(-5, 5, length.out = 100)
grid <- expand.grid(x = x, y = y)
grid$E <- grid$x^2 + grid$y^2
E_matrix <- matrix(grid$E, nrow = 100, ncol = 100)

contour(x, y, E_matrix,
        levels = c(1, 4, 9, 16, 25),
        col = "steelblue",
        lwd = 2,
        main = "Contour Map of E(x, y) = x^2 + y^2",
        xlab = "x (east-west)",
        ylab = "y (north-south)")
points(3, 4, pch = 19, col = "tomato")
text(3, 4, "E = 25", pos = 4, col = "tomato")

```

![Figure: Contour map of E(x, y) = x^2 + y^2.](images/sea_level.png){fig.alt="A contour map shows level curves of the function E(x, y) = x squared plus y squared. The contours are concentric circles centered at the origin, with values increasing outward. Inner contours are labeled with smaller values such as 1 and 4, while outer contours are labeled with larger values such as 9, 16, and 25. A point on the outer contour is marked and labeled E = 25. The x-axis is labeled east-west and the y-axis north-south. The map illustrates that the function increases with distance from the center."}
...
1. **Apply the Product Rule and Chain Rule**
\[
...
2. **Simplify and solve for \(\frac{dL}{dT}\)**
\[
(L - 100) + (T - 5)\frac{dL}{dT} = 0
\]
\[
...
Implicit differentiation extends the Chain Rule to relationships that **can’t be untangled** into a single function.  
It’s the calculus tool for understanding how interconnected variables co-adjust—essential for studying **feedbacks** and **constraints** in environmental systems.
...
15. \( U'(s) = 3s^{-2/3} + 2s^{-3/2} \)

16. \( V'(t) = 18e^{3t} \)

17. \( W'(x) = 0.4x^3 - 0.75x^{-4} \)

18. \( X'(y) = 2\cos y + 3\sin y \)

19. \( Y'(x) = 4e^{x} + \dfrac{1}{x} \)

20. \( Z'(\theta) = 10\theta - 7\cos\theta + e^{\theta} \)
...
25. \( q'(y) = \dfrac{1}{3}y^{-2/3} \)

26. \( r'(x) = -35x^{-6} \)

27. \( m'(z) = \dfrac{1}{2}z^{-1/2} + \dfrac{1}{2}z^{-3/2} \)

28. \( n'(t) = 4t^7 - 9t^2 + 2 \)

29. \( k'(x) = \dfrac{3}{2}x^{-7} + 18x \)

30. \( P'(t) = 100e^{t} \)

31. \( Q'(r) = -0.2e^{-0.2r} \)

32. \( R'(\theta) = \dfrac{1}{\theta} \)
...
37. \( W'(s) = \dfrac{3}{4}s^{-1/4} - \dfrac{8}{3}s^{-5/3} \)
...
41. \( A'(u) = 2\cos u - 3\sin u \)

42. \( B'(\theta) = -\cos\theta + \sin\theta \)

43. \( C'(x) = 63x^8 - 3x^{-10} \)

44. \( D'(t) = -16e^{2t} - 5e^{-t} \)

45. \( E'(s) = \dfrac{1}{s} - 4e^{s} \)

46. \( F'(y) = \dfrac{3}{2}y^{-1/2} - 10y^{3/2} \)

47. \( G'(x) = \dfrac{7}{3}x^{4/3} + \dfrac{35}{3}x^{-10/3} \)

48. \( H'(z) = 0.05z^4 - 60z^{-6} \)

49. \( J'(t) = 2\cos t - 0.5\sec^2 t \)

50. \( K'(x) = e^{x} + \dfrac{1}{x} - 4x^3 \)
...
Rewrite as \( \sin\theta \, e^{-\theta} \):
\[
s'(\theta) = \cos\theta e^{-\theta} - \sin\theta e^{-\theta}
\]
...
Rewrite as \( (e^x+x)x^{-1/2} \):
\[
v'(x) = (e^x+1)x^{-1/2} - \tfrac12(e^x+x)x^{-3/2}
\]
...
\[
g'(t) = \tfrac{1}{2}(5t^2-4)^{-1/2}\cdot 10t
= \frac{5t}{\sqrt{5t^2-4}}
\]
...
\[
u'(x) = \tfrac{1}{3}(x^2+4x-1)^{-2/3}(2x+4)
= \frac{2x+4}{3(x^2+4x-1)^{2/3}}
\]
...
<details>
<summary><strong>Click to reveal solutions (with rules identified)</strong></summary>
...
\[
g'(t) = \frac{u'v - uv'}{v^2}
= \frac{\frac{2t}{1+t^2}t^3 - \ln(1+t^2)(3t^2)}{t^6}
\]
...
\[
p'(y) =
\frac{5e^{5y}(y^2+4)^{1/2} - e^{5y}\left(\tfrac12\right)(y^2+4)^{-1/2}(2y)}
{y^2+4}
\]
...
\[
u'(x) =
\frac{3\cos(3x)\ln(x^2+1) - \sin(3x)\left(\frac{2x}{x^2+1}\right)}
{[\ln(x^2+1)]^2}
\]
...
\[
w'(z) =
\frac{(e^{\sin z}\cos z + 2z)(\tan z)^{1/2}
- (e^{\sin z}+z^2)\left(\tfrac12\right)(\tan z)^{-1/2}\sec^2 z}
{\tan z}
\]
...
<details>
<summary><strong>Click to reveal solutions (implicit differentiation)</strong></summary>
...
Differentiate:
\[
3x^2 + 3y^2\frac{dy}{dx} = 9\big(x\frac{dy}{dx} + y\big)
\]
...
Differentiate (chain rule + product rule):
\[
\cos(xy)\big(x\frac{dy}{dx} + y\big) = 1
\]
...
Differentiate:
\[
e^{x+y}(1 + \frac{dy}{dx}) = 2x
\]

Solve:
\[
\frac{dy}{dx} = \frac{2x}{e^{x+y}} - 1
\]
...
Differentiate:
\[
\frac{1}{x^2+y^2}(2x + 2y\frac{dy}{dx}) = 3
\]

Multiply through:
\[
2x + 2y\frac{dy}{dx} = 3(x^2+y^2)
\]

Solve:
\[
\frac{dy}{dx} = \frac{3(x^2+y^2) - 2x}{2y}
\]
...
Differentiate:
\[
2y\frac{dy}{dx} + e^y\frac{dy}{dx} = 1
\]
...
<details>
<summary><strong>Click to reveal solutions (rates of change in context)</strong></summary>
...
\[
N'(t) = 80(-1)(1+5e^{-0.4t})^{-2}\big(5(-0.4)e^{-0.4t}\big)
\]
...
Product rule:
\[
H'(t) = 4(t+1)^{-1/2} + (100+4t)\left(-\tfrac12\right)(t+1)^{-3/2}
\]
...
e^{-0.03t}
\left[
3\pi\cos\!\left(\tfrac{2\pi t}{12}\right)
-0.03\big(120+18\sin\!\tfrac{2\pi t}{12}\big)
\right]
}
\]
