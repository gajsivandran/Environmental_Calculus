...
![Figure: Classifying extrema using critical points, concavity, and endpoints.](images/classifying_optima.png){fig.alt="Graph of a smooth function over a finite interval showing how to classify extrema. The curve starts at a left endpoint that is labeled as a global minimum, rises to a local maximum where the graph is concave down, then decreases to a local minimum where the graph is concave up, and finally increases to a right endpoint labeled as a global maximum. The figure highlights that local extrema occur at critical points and are classified using concavity, while global extrema can also occur at endpoints."}
...
![Figure: Why critical points alone may not determine optimal solutions.](images/feasible_domain_optima.png){fig.alt="Graph illustrating optimization with a restricted domain. The left side of the graph, where x is negative, is shaded and labeled as an infeasible region. A critical point in this region is marked but labeled as not feasible. The feasible region begins at x equals 0 and is shaded differently. Within the feasible region, there is a flat inflection point near x equals 2 where the slope is zero but there is no maximum or minimum. The graph continues upward to a right endpoint that is labeled as the global maximum. The figure emphasizes that only points within the feasible domain and endpoints should be considered when identifying optimal solutions."}
...
![Figure: Optimizing planting density using a function and its derivative.](images/planting_density_optimization.png){fig.alt="Two stacked graphs showing carbon uptake as a function of planting density and its derivative. The top graph shows total carbon uptake increasing with planting density, reaching a maximum at 50 trees per hectare, and then gradually decreasing. A point at x equals 50 is labeled as the maximum. The bottom graph shows the derivative, representing marginal change in carbon uptake, which decreases and crosses zero at x equals 50. This indicates that the maximum in the top graph occurs where the derivative equals zero. The horizontal axis represents planting density and the vertical axes represent carbon uptake and its rate of change."}
...
![Figure: Minimizing pollution concentration through dilution.](images/pollution_dilution.png){fig.alt="Graph showing pollution concentration C(Q) decreasing as discharge Q increases from 0 to 50 cubic meters per second. The curve starts at a high concentration at low discharge, labeled as an endpoint maximum, and steadily declines without turning points. The derivative is indicated to be negative everywhere, meaning there are no internal critical points. The lowest concentration occurs at the right endpoint near Q equals 50, labeled as the endpoint minimum and best allowed flow. The horizontal axis represents discharge and the vertical axis represents pollution concentration."}
...
![Figure: Maximizing reservoir release benefits.](images/reservoir_release_benefit.png){fig.alt="Graph of total benefit B(R) versus reservoir release R from 0 to 200 cubic meters per second. The curve increases rapidly at first, then levels off and reaches a maximum near R equals 150, which is marked with a point and labeled as the maximum. After this point, the benefit slightly decreases as release continues to increase. The horizontal axis represents release and the vertical axis represents total benefit."}
...
![Figure: Maximum sustainable yield in logistic growth.](images/logistic_yield.png){fig.alt="Graph of sustainable yield Y(P) versus population size P for a logistic growth model. The curve is a downward-opening parabola that starts at zero, rises to a maximum at P equals K over 2, and then decreases back to zero as P approaches the carrying capacity. The peak at P equals K over 2 is marked and labeled as the maximum sustainable yield. The horizontal axis represents population size and the vertical axis represents sustainable yield."}
...
![Figure: Optimal indoor temperature balancing energy and comfort costs.](images/temperature_optimization.png){fig.alt="Graph of total cost C(T) versus indoor temperature from about 8 to 26 degrees Celsius. The curve is U-shaped, with higher costs at both low and high temperatures and a minimum point near 19 degrees Celsius. A vertical dashed line and a point mark the minimum cost at T equals 19 degrees Celsius, indicating the optimal indoor temperature that balances energy use and comfort."}
