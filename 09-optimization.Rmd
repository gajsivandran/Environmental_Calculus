...
- **Optimal dam release:**  
  Determine flow levels that minimize flood risk while maximizing hydro power or maintaining habitat conditions.
...
```{r classify-optima-critical-concavity-endpoints, fig.cap="Classifying optima using critical points, concavity, and endpoints.", fig.alt="Graph of a smooth function over a finite interval from x equals negative 3 to x equals 8. The curve has one local maximum and one local minimum at interior critical points, both marked in red. The local maximum occurs where the graph is concave down, and the local minimum occurs where the graph is concave up. The two endpoints of the interval are marked in green and labeled as the global minimum and global maximum. The figure illustrates that local extrema are identified at critical points using concavity, while absolute extrema on a closed interval can occur at endpoints.", echo=FALSE, warning=FALSE, message=FALSE}
...
```{r planting-density-optimization-2panel, fig.cap="Optimizing planting density using a function and its derivative.", fig.alt="Two stacked graphs show carbon uptake as a function of planting density and its derivative. The top graph shows total carbon uptake increasing with planting density, reaching a maximum at 50 trees per hectare, and then decreasing for larger densities. The maximum point is marked with dashed horizontal and vertical guide lines. The bottom graph shows the derivative, representing the marginal change in carbon uptake. It is positive before 50 trees per hectare, zero at 50, and negative afterward. The figure illustrates that the maximum in the original function occurs where the derivative changes sign and equals zero.", warning=FALSE, message=FALSE, echo=FALSE}
...
1. **Competition for light:** At high densities, trees shade each other, reducing photosynthesis per tree faster than the additional trees can compensate.
2. **Competition for water and nutrients:** More trees draw from the same soil resources, stunting individual growth and potentially increasing mortality.
...
```{r reservoir-release-benefit-max, fig.cap="Maximizing total benefit from reservoir release.", fig.alt="Graph of total benefit B of R as a function of reservoir release from 0 to 200 cubic meters per second. The curve rises at first, then levels off and reaches a maximum at an interior point, after which it begins to decline. The maximum point is marked with a red dot and dashed horizontal and vertical guide lines. The figure illustrates an optimization problem in which the best reservoir release occurs at a critical point where total benefit is highest.", warning=FALSE, message=FALSE, echo=FALSE}
...
```{r logistic-yield-maximum, fig.cap="Maximum sustainable yield in a logistic growth model.", fig.alt="Graph of sustainable yield Y of P as a function of population size from 0 to the carrying capacity. The curve is hump-shaped, starting at zero when the population is zero, increasing to a maximum at half the carrying capacity, and then decreasing back to zero as the population approaches the carrying capacity. The maximum point at P equals K over 2 is marked with a red dot and dashed guide lines. The figure illustrates that the maximum sustainable yield occurs at an intermediate population size rather than at the largest population.", echo=FALSE, message=FALSE, warning=FALSE}
...
![Figure: Cost minimization subject to an environmental constraint.](images/cost_constraint.png){fig.alt="Two stacked graphs showing cost and pollutant levels as functions of treatment intensity x. The top graph shows cost increasing with treatment intensity, with a marked feasible minimum at a specific value x equals x minimum. The bottom graph shows pollutant concentration decreasing as treatment intensity increases, with a horizontal target line indicating the required maximum pollutant level. The point where the pollutant curve meets the target corresponds to x equals x minimum, showing that the lowest cost satisfying the environmental constraint occurs at the minimum treatment level that meets the pollutant target."}
...
The plot below shows the cost curve and the optimal set point.

```{r indoor-temperature-tradeoff, fig.cap="Optimal indoor temperature from an energy–comfort tradeoff.", fig.alt="Graph of total cost C of T as a function of indoor temperature from 8 to 26 degrees Celsius. The curve is U-shaped, with costs high at both low and high temperatures and a minimum near 19 degrees Celsius. A vertical dashed line and a red point mark the minimum cost. The figure illustrates that the optimal indoor temperature balances energy costs and comfort costs, producing the lowest total cost at an intermediate temperature.", warning=FALSE, message=FALSE, echo=FALSE}
...
The extrema (maximum value approached) is:
\[
\sup E(t) = 15.
\]
...
1. Find the spacing \(d\) that **maximizes** under-story light availability.  
