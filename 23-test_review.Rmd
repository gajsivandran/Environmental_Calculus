...
\lim_{x\to -\infty} h_1(x)= -\infty
\]

Heading to the right of the function we have the asymptote:
\[
\text{Horizontal asymptote: } y=1
\]

However, when we go to the left, towards -\infty, we need to use L'Hopital's
...
```{r vertical-asymptote-plot, fig.cap="Function with a vertical asymptote at x = 2.", fig.alt="Graph of the function f(x) equals 1 divided by the quantity x minus 2. The graph has two separate branches on either side of x equals 2 and a dashed vertical line marking the asymptote. As x approaches 2 from the left, the function decreases without bound, and as x approaches 2 from the right, the function increases without bound. The figure illustrates that the function is undefined at x equals 2 and has a vertical asymptote there.", echo=FALSE, message=FALSE, warning=FALSE}
...
plot(x_left, f(x_left),
     ...
     xlab = "x",
     ylab = "f(x)",
     main = paste("Vertical Asymptote at x =", a))
...
# Add a label
text(a, 9, labels = paste("x =", a), pos = 4)
```
...
```{r exp-vs-polynomial-growth, fig.cap="Comparison of exponential and polynomial growth.", fig.alt="Graph comparing two functions over the interval from x equals 0 to 5. The exponential function e to the x starts slowly and then increases rapidly, eventually rising much faster than the polynomial function x squared. The curve for x squared increases steadily but remains below the exponential curve for larger values of x. The figure illustrates that exponential growth eventually outpaces polynomial growth.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r derivative-sign-chart-visual, fig.cap="Using critical points to determine increasing and decreasing behavior.", fig.alt="Graph of the function f(x) equals x cubed minus 3x over the interval from about negative 2.5 to 2.5. The curve has a local maximum at x equals negative 1 and a local minimum at x equals 1, both marked with points and vertical dashed lines. The function increases to the left of x equals negative 1, decreases between negative 1 and 1, and increases again to the right of x equals 1, illustrating how critical points divide the graph into intervals of increasing and decreasing behavior.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r curve-sketching-cubic-labeled, fig.cap="Curve sketch of a cubic showing extrema, inflection point, and concavity.", fig.alt="Graph of the function f(x) equals x cubed minus 3x over the interval from about negative 2.6 to 2.6. The curve has a local maximum at the point negative 1 comma 2 and a local minimum at the point 1 comma negative 2. An inflection point is marked at the origin, where the graph changes concavity. The left half of the graph is shaded to indicate concave down behavior for x less than 0, and the right half is shaded to indicate concave up behavior for x greater than 0. The figure illustrates how critical points and concavity together describe the overall shape of the curve, while also noting that the function has no global maximum or minimum because it is unbounded.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r asymptote-skeleton-step1, fig.cap="Skeleton graph showing vertical and horizontal asymptotes.", fig.alt="Coordinate plane showing the asymptotes of a rational function before the full curve is drawn. Two dashed vertical lines mark vertical asymptotes at x equals negative 1 and x equals 1. A dashed horizontal line marks a horizontal asymptote at y equals 2. Arrows near x equals negative 1 indicate the function approaches positive infinity from both sides of that asymptote, while arrows near x equals 1 indicate the function approaches negative infinity from both sides. The figure provides the asymptotic structure used to sketch the full graph.", echo=FALSE, message=FALSE, warning=FALSE}
...
# Indicate direction near the vertical asymptotes
...
```{r asymptote-skeleton-step2, fig.cap="Adding an anchor point to the asymptote skeleton.", fig.alt="Graph showing the same asymptotes as before, with dashed vertical lines at x equals negative 1 and x equals 1 and a dashed horizontal line at y equals 2. In addition, a point is marked at the coordinate 0 comma 2, labeled as an anchor point. This point lies on the horizontal asymptote and helps guide how the curve will be drawn relative to the asymptotes.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r asymptote-skeleton-step3, fig.cap="Adding increasing and decreasing behavior to the asymptote skeleton.", fig.alt="Graph showing the asymptote skeleton of a rational function with dashed vertical lines at x equals negative 1 and x equals 1, a dashed horizontal line at y equals 2, and an anchor point at 0 comma 2. Two additional critical points are marked, one to the left of x equals negative 1 labeled local minimum and one to the right of x equals 1 labeled local maximum. Along the bottom of the graph, arrows and labels indicate intervals where the function is increasing or decreasing. The function decreases, then increases before the left asymptote, decreases between the asymptotes, increases just to the right of the right asymptote, and then decreases again. The figure shows how derivative information helps organize the overall shape before drawing the full curve.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r asymptote-skeleton-step4, fig.cap="Adding concavity and inflection points to the asymptote skeleton.", fig.alt="Graph showing the asymptote skeleton of a rational function with dashed vertical lines at x equals negative 1 and x equals 1, a dashed horizontal line at y equals 2, and shaded regions indicating concavity. Blue-tinted regions mark intervals where the graph is concave down, and green-tinted regions mark intervals where the graph is concave up. Three inflection points are marked and labeled: one on the left branch, one at x equals 0 on the middle branch, and one on the right branch. The point at 0 comma 2 is also marked as an anchor point. The figure shows how second-derivative information identifies where the curve bends upward or downward and where concavity changes.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r rational-practice1-annotated, fig.cap="Annotated graph of a rational function showing vertical and horizontal asymptotes.", fig.alt="Graph of the function f(x) equals 1 minus 1 divided by the quantity x minus 2. The graph has two branches separated by a vertical asymptote at x equals 2, shown as a dashed line. A horizontal asymptote at y equals 1 is also shown as a dashed line. The left branch lies below the horizontal asymptote and decreases toward negative infinity as x approaches 2 from the left. The right branch lies above the horizontal asymptote and decreases toward negative infinity as x approaches 2 from the right, while both branches approach y equals 1 as x moves away from the asymptote.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r polynomial-practice2-annotated, fig.cap="Annotated graph of a cubic polynomial showing turning points.", fig.alt="Graph of the function f(x) equals the quantity x plus 1 squared times the quantity x minus 2. The curve touches the x-axis at x equals negative 1 and crosses the x-axis at x equals 2. A local maximum is marked at x equals negative 1, where the graph flattens and turns downward, and a local minimum is marked near x equals 1, where the graph turns upward. A dashed horizontal line indicates the x-axis. The figure illustrates how critical points correspond to turning behavior in a cubic function.", echo=FALSE, message=FALSE, warning=FALSE}
...
# (For this polynomial, critical points occur at x = -1 and x = 1/3)
...
```{r polynomial-practice3-inflection, fig.cap="Annotated graph showing an inflection point and concavity change.", fig.alt="Graph of the function f(x) equals x cubed plus 2 over the interval from negative 3 to 3. The curve passes smoothly through the point 0 comma 2, which is marked and labeled as an inflection point. A dashed vertical line highlights x equals 0. To the left of this point the graph is concave down, and to the right it is concave up, showing a change in curvature without a local maximum or minimum.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r rational-practice4-annotated, fig.cap="Annotated graph of a rational function with asymptotes and curvature change.", fig.alt="Graph of the function f(x) equals 2 plus x cubed divided by the quantity x squared minus 1 squared. The graph has three branches separated by vertical asymptotes at x equals negative 1 and x equals 1, shown as dashed lines. A horizontal asymptote at y equals 2 is also shown. A point at 0 comma 2 is marked on the middle branch. A dashed vertical line at x equals 0 indicates a change in curvature, showing an inflection point on the central branch. The figure illustrates asymptotic behavior together with a curvature change in the interior of the domain.", echo=FALSE, message=FALSE, warning=FALSE}
